<template>
    <div class="planet-card">
        ...
        <div class="temperature">
            <p v-if="planetInfo.meanTemp" title="Surface temperature">平均温度: 
            ...
            <p v-if="planetInfo.minTemp" title="Surface temperature">最低: 
            ...
            <p v-if="planetInfo.maxTemp" title="Surface temperature">最高: 
        ...
        <div class="info">
            ...
            <ul>
                <li>半径: <span class="value">{{ planetInfo.radius }} km</span></li>
                <li v-if="planetInfo.timesLarger && planetInfo.timesLarger !== -1">
                    <span v-if="planetInfo.timesLarger >= 1"> 是地球的 <span class="value">{{ planetInfo.timesLarger }}</span> 倍</span>
                    <span v-else> 是地球的 <span class="value">1/{{ (1 / planetInfo.timesLarger).toFixed(2) }}</span></span>
                </li>
                <li v-if="planetInfo.distanceFromSun && planetInfo.distanceFromSun !== -1">距离太阳的距离: <span class="value">{{ planetInfo.distanceFromSun }} AU</span></li>
                <li v-if="planetInfo.distance">距离{{ planetInfo.orbitObject.charAt(0).toUpperCase() + planetInfo.orbitObject.slice(1) }}的距离: <span class="value">{{ planetInfo.distance }} km</span></li>
                <li v-if="planetInfo.year">自转周期: <span class="value">{{ planetInfo.year }}</span></li>
                <li v-if="planetInfo.day">1 行星日: <span class="value">{{ planetInfo.day }}</span></li>
                <li v-if="planetInfo.moons != null">卫星: <span class="value">{{ planetInfo.moons }}</span></li>
            </ul>
        </div>
    </div>
</template>
